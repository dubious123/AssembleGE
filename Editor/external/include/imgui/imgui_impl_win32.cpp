...
IMGUI_IMPL_API LRESULT ImGui_ImplWin32_WndProcHandler(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_LBUTTONDOWN:
	case WM_LBUTTONDBLCLK:	  // case WM_NCLBUTTONDOWN:case WM_NCLBUTTONDBLCLK:
	case WM_RBUTTONDOWN:
	case WM_RBUTTONDBLCLK:	  // case WM_NCRBUTTONDOWN:case WM_NCRBUTTONDBLCLK:
	case WM_MBUTTONDOWN:
	case WM_MBUTTONDBLCLK:	  // case WM_NCMBUTTONDOWN:case WM_NCMBUTTONDBLCLK:
	case WM_XBUTTONDOWN:
	case WM_XBUTTONDBLCLK:	  // case WM_NCXBUTTONDOWN:case WM_NCXBUTTONDBLCLK:
	{
		ImGuiMouseSource mouse_source = GetMouseSourceFromMessageExtraInfo();
		int				 button		  = 0;
		...
		if (bd->MouseButtonsDown == 0 && ::GetCapture() == nullptr && (msg > WM_NCXBUTTONDBLCLK))
			::SetCapture(hwnd);
		bd->MouseButtonsDown |= 1 << button;
		io.AddMouseSourceEvent(mouse_source);
		io.AddMouseButtonEvent(button, true);

		// if(msg <= WM_NCXBUTTONDBLCLK) DefWindowProcW(hwnd, msg, wParam, lParam);
		return 0;
	}
	...
	case WM_XBUTTONUP:	  // case WM_NCXBUTTONUP:
	{
		...
		// if(msg <= WM_NCXBUTTONUP) DefWindowProcW(hwnd, msg, wParam, lParam);
		return 0;
	}
	case WM_MOUSEWHEEL:
