	...
	init() noexcept
	{
		...
		on_ctx{
			...
			identity{ age::graphics::render_pipeline::forward_plus::point_light_desc{
				...
				.intensity = 3.0f } }
				| AGE_LAMBDA((auto&& desc), { return i_ctx.get_render_pipeline().add_point_light(FWD(desc), false); })
				| AGE_FUNC(i_ctx.get_point_light_id_vec().emplace_back),
			...
			identity{ age::graphics::render_pipeline::forward_plus::point_light_desc{
				...
				.intensity = 3.0f } }
				| AGE_LAMBDA((auto&& desc), { return i_ctx.get_render_pipeline().add_point_light(FWD(desc), true); })
				| AGE_FUNC(i_ctx.get_point_light_id_vec().emplace_back),
			...
			identity{ age::graphics::render_pipeline::forward_plus::point_light_desc{
				...
				.intensity = 3.0f } }
				| AGE_FUNC(i_ctx.get_render_pipeline().add_point_light)
				| AGE_FUNC(i_ctx.get_point_light_id_vec().emplace_back),
			...
			identity{ age::graphics::render_pipeline::forward_plus::spot_light_desc{
				...
				.cos_inner = 0.96f,
				.cos_outer = 0.87f } }
				| AGE_LAMBDA((auto&& desc), { return i_ctx.get_render_pipeline().add_spot_light(FWD(desc), true); })
			...
			AGE_LAMBDA(
				(),
				{
					auto add_obj = [&](float3 pos, float3 scale, float4 quat = age::g::quaternion_identity) {
						i_ctx.get_obj_id_vec().emplace_back(
							i_ctx.get_render_pipeline().add_object(
