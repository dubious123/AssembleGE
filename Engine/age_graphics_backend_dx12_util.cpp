...
namespace age::graphics
{
	...
	to_string(D3D12_RESOURCE_DIMENSION dim) noexcept
	{
		...
		default:
		{
			AGE_UNREACHABLE("invalid D3D12_RESOURCE_DIMENSION : {}", std::to_underlying(dim));
		}
		}
	}

	constexpr std::string_view
	to_string(DXGI_FORMAT format) noexcept
	{
		switch (format)
		{
		...
		default:
		{
			AGE_UNREACHABLE("invalid DXGI_FORMAT : {}", std::to_underlying(format));
		}
		}
	}
	...
	format_size(DXGI_FORMAT format) noexcept
	{
		...
		default:
		{
			AGE_UNREACHABLE("unsupported DXGI_FORMAT for bytes_per_pixel: {}", to_string(format));
		}
		}
	}

	constexpr DXGI_FORMAT
	dx12_format(e::texture_format format) noexcept
	{
		return static_cast<DXGI_FORMAT>(to_idx(format));
	}
