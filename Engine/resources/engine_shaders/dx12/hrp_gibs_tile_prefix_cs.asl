...
//
//{
//	const uint32 tile_id_base = group_id * GIBS_CELL_PREFIX_EPG;
//
...
//	uint32 local_sum = 0u;
//
//	rw_byte_array<gibs_tile_surfel_entry> tile_entry_arr = gibs::tile::surfel_entry_rw_arr(data);
//
//	uint32 local_prefix_arr[GIBS_CELL_PREFIX_EPT];
//
//	expand_all()
//
...
//
//		const gibs_tile_surfel_entry entry = tile_entry_arr[tile_id];
//
...
//	{
//		group_offset_tmp = gibs::tile::alloc_surfel(data, local_offset + local_sum);
//	}
//	const uint32 group_offset = wave_read_lane_at(group_offset_tmp, GIBS_CELL_PREFIX_TPG - 1);
//
//	expand_all()
//
//	for (uint32 i = 0; i < GIBS_CELL_PREFIX_EPT; ++i)
...
//		if (tile_id >= data.tile_count_total) { break; }
//
//		gibs_tile_surfel_entry entry = tile_entry_arr[tile_id];
//
//		entry.offset = group_offset
//					 + local_offset
//					 + local_prefix_arr[i];
//
//		entry.surfel_count = 0u;
//
...

{
	...
	for (uint32 i = 0; i < GIBS_CELL_PREFIX_EPT; ++i)
	{
		...
		local_sum += tile_entry_arr[tile_id].surfel_count;
	}
	...
	for (uint32 i = 0; i < GIBS_CELL_PREFIX_EPT; ++i)
	{
		...
		gibs_tile_surfel_entry entry = tile_entry_arr[tile_id];
		const uint32		   count = entry.surfel_count;
