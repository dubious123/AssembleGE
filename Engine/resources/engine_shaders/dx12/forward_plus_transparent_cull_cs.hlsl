...
[numthreads(TRANSPARENT_CULL_THREAD_COUNT, 1, 1)]
void main_cs(uint32 dispatch_thread_id : SV_DispatchThreadID)
{
    ...
    if (transparent_render_data_id < transparent_object_render_data_count)
    {
        ...
        const float4x4 world_mat = affine_transformation(obj_data.pos, quat, float3(obj_data.scale));
        ...
        for (uint32 i = 0; i < 6; ++i)
        {
            ...
            const float distance = dot(frustum_planes[i], float4(center_world, 1));
