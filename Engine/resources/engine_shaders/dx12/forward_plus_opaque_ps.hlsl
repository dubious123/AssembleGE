...
float4
main_ps__(opaque_ms_to_ps fragment): SV_Target0
{
    ...
    for (uint i = 0; i < unified_light_count; ++i)
    {
        lighting += calc_blinn_phong_light_color(unified_sorted_light_buffer_srv[i], fragment.world_pos, surface_normal, view_dir);
    }
...
main_ps_(opaque_ms_to_ps fragment): SV_Target0
{
    ...
    for (uint32 w = word_begin; w <= word_end; ++w)
    {
        ...
        while (bit_mask != 0)
        {
            ...
            const t_unified_light_id light_id = sort_buffer_srv[LIGHT_SORT_CS_SORT_VALUES_OFFSET + sorted_id];
           
            lighting += calc_blinn_phong_light_color(unified_sorted_light_buffer_srv[light_id], fragment.world_pos, surface_normal, view_dir);
        }
    }
