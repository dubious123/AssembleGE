...

{
	...
	array<gibs_gi_resolve_cell> gi_resolve_cell_arr = gibs_load_gi_resolve_cell_arr(data);
	...
	{
		const int32_2 cell_idx = px / GIBS_GI_RESOLVE_CELL_SIZE;
		// const int32	  cell_id	  = cell_idx.x + cell_idx.y * data.tile_count_w * GIBS_GI_RESOLVE_CELL_DIM;
		const int32_2 cell_extent = int32_2(data.tile_count_w * GIBS_GI_RESOLVE_CELL_DIM, data.tile_count_h * GIBS_GI_RESOLVE_CELL_DIM);
		...
		for (int32 dy = -r.y; dy <= r.y; ++dy)
		{
			...
			{
				const int32_2 cell_tap = cell_idx + int32_2(dx, dy);

				if (any(cell_tap < 0) or any(cell_tap >= cell_extent)) { continue; }

				for (int32 i = 0; i < GIBS_GI_RESOLVE_SAMPLES_PER_CELL; ++i)
				{
					const uint32 cell_id = (cell_tap.x + cell_tap.y * data.tile_count_w * GIBS_GI_RESOLVE_CELL_DIM) * GIBS_GI_RESOLVE_SAMPLES_PER_CELL
										 + i;

					const gibs_gi_resolve_cell cell = gi_resolve_cell_arr[cell_id];

					if (cell.coverage == 0.h) { continue; }

					const float3  sample_irradiance = decode_r11g11b10(cell.irradiance_r11g11b10);
					const int32_2 sample_px			= cell_tap * GIBS_GI_RESOLVE_CELL_SIZE
													+ cell.get_sample_px();

					const float pos_rel	   = length_sq(px - sample_px);
					const float pos_weight = 1.f / (1.f + pos_rel * (1.f / GIBS_GI_RESOLVE_CELL_SIZE));

					const float3 sample_normal = decode_oct_snorm16(cell.normal_oct_snorm16);
					const float	 sample_z_lin  = cell.z_lin;
