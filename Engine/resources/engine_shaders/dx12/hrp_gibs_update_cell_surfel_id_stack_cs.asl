...
{
	rw_stack<uint32> dead_stack = gibs::cell::dead_id_stack(data);
...

{
	...
	rw_stack<uint32> alive_stack_prev		= gibs::cell::alive_id_stack_prev(data);
	...
	if (surfel.recycle_data.frame_since_ref() >= 0xff)
	{
		kill_surfel = true;
	}
	...
	rw_stack<uint32> alive_stack_curr = gibs::cell::alive_id_stack_curr(data);
