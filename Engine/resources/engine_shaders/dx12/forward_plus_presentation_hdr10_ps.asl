...
main_ps(float4 pos sv_position) sv_target_0
{
	...
	if (ddgi_enabled())
	{
		...
		{
			texture_2d<float3> irradiance_atlas = global_resource_buffer[data.irradiance_atlas_curr_srv_id];
		...
		{
			texture_2d<float3> irradiance_atlas = global_resource_buffer[data.irradiance_atlas_prev_srv_id];
			float2			   debug_uv			= pos.xy * inv_backbuffer_size;
