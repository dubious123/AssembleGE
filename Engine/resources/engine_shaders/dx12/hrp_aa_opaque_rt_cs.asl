...
rt_trace(float2 px, ray_desc desc, inout ray_query<ray_flag> query)
{
	...
	else
	{
		...
		{
			const float4 irradiance = gist::sample_irradiance<true, false>(gist::load_data(), invalid_id_uint32, surface_data.world_pos, surface_data.vertex_normal, invalid_id_uint32);
