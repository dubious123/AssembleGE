...

{
	...
	const uint32 ideal_ray_count_total = gibs::cell::ideal_ray_count_total(data);
	const float	 ray_count_factor	   = saturate(float(gibs::ray_budget(data)) / ideal_ray_count_total);

	rw_byte_array<uint16> ray_count_arr		   = gibs::cell::ray_count_rw_arr(data);
	rw_byte_array<uint32> ray_count_prefix_arr = gibs::cell::ray_count_prefix_rw_arr(data);

	const uint32 alive_count_total = gibs::cell::alive_count_curr(data);
	...
	{
		group_offset_tmp = gibs::cell::alloc_ray(data, local_offset + local_sum);
	}
