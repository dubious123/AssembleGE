...
groupshared transparent_as_to_ms as_out;
...

{
	...
	if (render_data_id < load_transparent_meshlet_render_data_count())
	{
		const transparent_meshlet_render_data render_data = load_transparent_meshlet_render_data(render_data_id);
		const object_data					  obj_data	  = load_object_data(render_data.object_id);

		const uint32 meshlet_idx = render_data.meshlet_id;

		const mesh_header	 msh_header	  = read_mesh_header<transparent_meshlet_render_data>(render_data);
