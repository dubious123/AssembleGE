		...
		out indices uint32_3 ms_out_triangle_arr[126])
{
	...
	[unroll(2)]
    for (uint nth_vertex = group_thread_id; nth_vertex < vertex_count; nth_vertex += 32)
    {
	...
	[unroll(4)]
    for (uint nth_primitive = group_thread_id; nth_primitive < primitive_count; nth_primitive += 32)
    {
