...

{
	...
	const uint32 group_sum_count	 = ceil(gibs::tile::alive_count_curr(data), AGE_WAVE_SIZE);
	uint32		 local_ray_count_sum = 0u;

	const rw_byte_array<uint16> ideal_ray_count_wave_sum_arr = gibs::tile::ideal_ray_count_wave_sum_rw_arr(data);
	...
	{
		gibs::tile::atomic_add_ideal_ray_count(data, ray_count_wave_sum);
	}
}