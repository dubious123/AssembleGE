...

{
	...
	{
		attr_branch()
		if (gibs::debug::freeze_spawn_kill(data)) { break; }

		gibs::cell_surfel_spawn_data spawn_data;
		if (gibs::cell::load_surfel_spawn_data(data, cell_id, spawn_data) is_false) { break; }

		const gibs_ray_hit_result ray_hit		  = ray_hit_buffer[spawn_data.ray_id];
