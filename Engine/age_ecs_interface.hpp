...
namespace age::ecs
{
	...
	struct interface_game
	{
		...
		interface_game(auto&& arg) : game{ FWD(arg) } { }
	...
	template <typename t>
	interface_game(t&&) -> interface_game<t>;
	...
	struct i_entity_storage
	{
		...
		using t_entity_block_idx = typename t_ent_storage::t_entity_block_idx;
		using t_entity_block	 = typename t_ent_storage::t_entity_block;
		...
		{
			return entity_storage.foreach_block(FWD(block_query), FWD(sys));
		}
	...
	template <typename t_entity_block>
	struct i_entity_block
	{
		using t_ent_block			= std::remove_reference_t<t_entity_block>;
		using t_ent_id				= typename t_ent_block::t_ent_id;
		using t_ent_block_idx		= typename t_ent_block::t_ent_block_idx;
		using t_local_entity_idx	= typename t_ent_block::t_local_entity_idx;
		using t_storage_cmp_idx		= typename t_ent_block::t_storage_cmp_idx;
		using t_archetype			= typename t_ent_block::t_archetype;
		using t_entity_count		= typename t_ent_block::t_entity_count;
		using t_capacity			= typename t_ent_block::t_capacity;
		using t_local_cmp_idx		= typename t_ent_block::t_local_cmp_idx;
		using t_component_count		= typename t_ent_block::t_component_count;
		using t_component_size		= typename t_ent_block::t_component_size;
		using t_component_offset	= typename t_ent_block::t_component_offset;
		using t_cmp_offset_arr_base = typename t_ent_block::t_cmp_offset_arr_base;
		using t_cmp_size_arr_base	= typename t_ent_block::t_cmp_size_arr_base;
		using t_entity_id_arr_base	= typename t_ent_block::t_entity_id_arr_base;

		t_entity_block& entity_block;

		FORCE_INLINE t_ent_block_idx&
		entity_block_idx()
		{
			return entity_block.entity_block_idx();
		}

		FORCE_INLINE t_entity_count&
		entity_count()
		{
			return entity_block.entity_count();
		}

		FORCE_INLINE t_capacity&
		capacity()
		{
			return entity_block.capacity();
		}

		FORCE_INLINE t_component_count&
		component_count()
		{
			return entity_block.component_count();
		}

		FORCE_INLINE t_archetype&
		local_archetype()
		{
			return entity_block.local_archetype();
		}

		FORCE_INLINE t_cmp_size_arr_base&
		component_size_arr_base()
		{
			return entity_block.component_size_arr_base();
		}

		FORCE_INLINE t_cmp_offset_arr_base&
		component_offset_arr_base()
		{
			return entity_block.component_offset_arr_base();
		}

		FORCE_INLINE t_entity_id_arr_base&
		entity_id_arr_base()
		{
			return entity_block.entity_id_arr_base();
		}

		FORCE_INLINE t_ent_id&
		ent_id(t_local_entity_idx ent_idx)
		{
			return entity_block.ent_id(ent_idx);
		}

		template <age::ecs::cx_component... t_cmp>
		FORCE_INLINE decltype(auto)
		get_component(const t_local_entity_idx local_ent_idx)
		{
			return entity_block.get_component<t_cmp...>(local_ent_idx);
		}
		...
		{
			return entity_block.foreach_entity(FWD(sys));
		}

		FORCE_INLINE bool
		is_full()
		{
			return entity_block.is_full();
		}

		FORCE_INLINE bool
		is_empty()
		{
			return entity_block.is_empty();
		}
	};
}	 // namespace age::ecs
