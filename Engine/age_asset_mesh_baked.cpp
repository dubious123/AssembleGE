...
{
	mesh_baked_header
	mesh_baked2::get_header() const noexcept
	{
	...
	mesh_baked2::get_global_vertex_index_buffer() const noexcept
	{
	...
	mesh_baked2::get_local_vertex_index_buffer() const noexcept
	{
		c_auto	header = get_header();
	...
	mesh_baked2::get_meshlet_buffer() const noexcept
	{
		c_auto	header = get_header();
	...
	mesh_baked2::get_mesh_header_buffer() const noexcept
	{
		c_auto	header = get_header();
	...
	FORCE_INLINE
	std::span<const t>
	mesh_baked2::get_vertex_buffer() const noexcept
	{
		c_auto	header = get_header();
	...
	mesh_baked2::gen_flat_index_arr() const noexcept
	{
	...
	mesh_baked2::gen_vertex_pos_arr() const noexcept
	{
		c_auto header = get_header();
...
namespace age::asset::mesh_baked::detail
{
	...
	load_common(std::string_view mesh_name) noexcept
	{
		...
		{
			h_mesh = asset::create_entry<e::kind::mesh_baked>(full_path);
		}
