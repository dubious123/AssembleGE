...
namespace age::util
{
	namespace detail
	{
		...
		template <uint32 precision, std::size_t n>
		constexpr void
		float_to_str_impl(char* buf, float value) noexcept
		{
			constexpr auto fixed_overhead = 1 + 1 + precision + 1;	  // sign + dot + decimals + null

			c_auto fmt = (std::abs(value) < threshold<n, fixed_overhead>())
		...
		constexpr void
		integral_to_str_impl(char* buf, std::size_t n, std::integral auto value) noexcept
		{
		...
		template <uint32 base, std::size_t digits>
		constexpr void
		to_str_with_prefix_impl(char* buf, std::size_t n, auto value, const char* prefix, std::size_t prefix_len) noexcept
		{
			...
			if constexpr (digits > 0)
			{
				...
				std::memset(buf + prefix_len, '0', pad);
				std::memcpy(buf + prefix_len + pad, tmp, written);
				buf[prefix_len + pad + written] = '\0';
			}
			else
			{
				c_auto written = static_cast<std::size_t>(ptr - tmp);
				std::memcpy(buf + prefix_len, tmp, written);
				buf[prefix_len + written] = '\0';
			}
		}

		template <typename t>
		constexpr bool
		from_str_impl(const char* buf, std::size_t len, t& value) noexcept
		{
			auto result		= t{};
			c_auto[ptr, ec] = std::from_chars(buf, buf + len, result);
	...
	float_to_str(char (&buf)[n], float value) noexcept
	{
		...
		detail::float_to_str_impl<precision, n>(buf, value);
	}
	...
	integral_to_str(char (&buf)[n], std::integral auto value) noexcept
	{
		...
		detail::integral_to_str_impl<base>(buf, n, value);
	}
	...
	integral_to_str(std::array<char, n>& buf, std::integral auto value) noexcept
	{
		...
		detail::integral_to_str_impl<base>(buf.data(), n, value);
	}
	...
	to_str(char (&buf)[n], t value, const char (&prefix)[p]) noexcept
	{
		...
		detail::to_str_with_prefix_impl<base, digits>(buf, n, value, prefix, len);
	}
	...
	{
		return detail::from_str_impl(buf, std::strlen(buf), value);
	}
